...
const RECORD_INTERVAL_MS = 550;
const MIN_TILE_DELTA = 3;
...
export default class PathRecorder extends BotScript {
    private recording = false;
    private buffer: Path = [];
    private lastRecorded: [number, number] | null = null;
    private intervalId: ReturnType<typeof setInterval> | null = null;
    private fromId: WalkNodeId;
    ...
    private getWorldPos(bot: Bot): [number, number] | null {
        return bot.api.webWalk.getPlayerWorldPos();
    }

    private tickRecord(bot: Bot) {
    ...
    private updateOutput() {
        const ta = document.getElementById('pathRecorderOutput') as HTMLTextAreaElement | null;
        if (!ta) {
            return;
        }
    ...
    static htmlSetup(base: HTMLElement) {
        ...
        startBtn.onclick = () => {
            (globalThis as unknown as { bot: Bot }).bot.start(
                PathRecorder.buildFromHtml(base)
            );
        };
        ...
        stopBtn.onclick = () => {
            (globalThis as unknown as { bot: Bot }).bot.stop();
        };
    ...
    static buildFromHtml(base: HTMLElement) {
        const from = (document.getElementById('pathRecorderFrom') as HTMLInputElement | null)?.value.trim() ?? 'hub_lumbridge';
        const to = (document.getElementById('pathRecorderTo') as HTMLInputElement | null)?.value.trim() ?? 'bank_al_kharid';
        return new PathRecorder(from as WalkNodeId, to as WalkNodeId);
    }

    override start(bot: Bot) {
        this.recording = true;
        this.buffer = [];
        ...
        this.fromId = (document.getElementById('pathRecorderFrom') as HTMLInputElement | null)?.value.trim() as WalkNodeId ?? this.fromId;
        this.toId = (document.getElementById('pathRecorderTo') as HTMLInputElement | null)?.value.trim() as WalkNodeId ?? this.toId;
        this.updateOutput();
